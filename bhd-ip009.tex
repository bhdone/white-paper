...
\begin{itemize}
    ...
    \item The foundation will receive an additional amount of the total amount already supplied on the current chain at one time
\end{itemize}
...
\begin{flushleft}
    We assume that there is a miner initialized $15_{PB}$ storage to do the mining. Assume that the current netspace of the entire network is $200_{PB}$ and the total supplied amount is $10,000_{BHD}$. Thus, according to the formula, the total amount that needs to be pledged per PB is $\frac{10,000}{200}=50$. The miner in order to obtain $100\%$ mining rewards, a total of $50_{BHD} * 15_{PB} = 750_{BHD}$ needs to be pledged to the chain.
\end{flushleft}
\textbf{$100\%$ rewards}
\begin{flushleft}
