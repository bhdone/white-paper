...
\subsection{Pledge}
\begin{flushleft}
    Miners are still need to deposit in order to get full reward from new block. But the way to determinate the deposit amount is changed. Also there are some properties are changed or added.
\end{flushleft}
\subsubsection{No fund to foundation}
\begin{flushleft}
    Foundation won't receive fund from new blocks, all funds from coinbase will be released to miner.
\end{flushleft}
\subsubsection{Netspace related}
\begin{flushleft}
    The amount to pledge for each TeraBytes will be adjusted according current network space. The minimum network space is 100PB, the network space is set to 100PB when it is less than 100PB. The required amount will be increased when the network space is increased.
\end{flushleft}
\begin{equation}
...
\begin{flushleft}
    When the amount of pledge is not enough for a miner to acquire all rewards from a new block. The miner will only get $15\%$ of the block rewards, the rest $85\%$ will be stored on the chain until the miner who has enought amount of pledge to claim all rewards.
\end{flushleft}
\subsubsection{Locking period and the weight of each pledge}
\begin{flushleft}
