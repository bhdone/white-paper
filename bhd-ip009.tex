...
\begin{flushleft}
    BitcoinHD1 will use a special (20-byte) accountID, it is filled by 0x23 of each byte. No one owns the private-key, and the consensus also prevent that no one will be able to make a new transaction that transfer BHD1 from this account even the fundation. All burned BHD1 will be sent to this account, and the amount required of mining will be re-calculated on next block.
\end{flushleft}
