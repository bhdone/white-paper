...
\end{flushleft}
\begin{itemize}
    \item The total amount of currency to be distributed in each block will be doubled after hard-fork
    \item Send the same amount of currency as the total amount of currency that has already been distributed to the foundation address
\end{itemize}
\subsection{Pledge}
\begin{flushleft}
    Miners still need to pledge currency to get the maximum mining profit, but the conditions for pledge have changed a bit.
\end{flushleft}
\subsubsection{No fund to foundation}
\begin{flushleft}
    The Foundation will no longer be funded in future blocks.
\end{flushleft}
\subsubsection{Netspace related}
\begin{flushleft}
    The amount that a miner needs to pledge will be related to its netspace and total supplied amount. To calculate the pledge amount for each tera bytes, use the formula below.
\end{flushleft}
\begin{equation}
...
\begin{flushleft}
    We assume that there is a miner with a netspace of $15_{PB}$. Assume that the current netspace of the entire network is $200_{PB}$ and the total supplied amount of currency is $10,000_{BHD}$. Thus, according to the formula, the total amount of currency that needs to be pledged per PB is $\frac{10,000}{200}=50$. The miner in order to obtain $100\%$ mining rewards, a total of $750_{BHD}$ needs to be pledged to the chain.
\end{flushleft}
\begin{flushleft}
...
\end{flushleft}
\begin{flushleft}
    Miner will receive $15\%$ rewards when the amount of pledge is less than $750_{BHD}$. The pledge amount also increases when the total supply is increased or the miner its netspace is increased.
\end{flushleft}
