...
const { people, titleLine, computer } = require('./lib/common.js');
...
add(titleLine(LINE_OFFSET, 0, PEOPLE_DIST - LINE_OFFSET, 0, '1. challenge x', DESC_COLOR));
add(titleLine(LINE_OFFSET, LINE_2ND_Y, PEOPLE_DIST - LINE_OFFSET, LINE_2ND_Y, '4. $X^{2^T}$ and P', DESC_COLOR, true));

const START_X = 7.5;
const LEN = 2;

add(titleLine(START_X, 0, START_X + LEN, 0, '2. fast computer', DESC_COLOR));
...
add(label(p1Props.left.x - MARGIN + TITLE_MARGIN, 1.5 - TITLE_MARGIN, 'Proof of time (VDF)', 'black', 'north west'));
